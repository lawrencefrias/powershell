$MicrosoftPolicyObj = $null
$MicrosoftPolicyObj = Get-ItemProperty -Path HKCU:\SOFTWARE\Policies\Microsoft\Edge -Name "UserDataDir"
$MicrosoftUserDir = $env:LOCALAPPDATA + "\Microsoft\Edge\User Data"
if ($MicrosoftPolicyObj -ne $null) {
    $MicrosoftUserDir = $MicrosoftPolicyObj.UserDataDir -replace "\$\{documents\}", [Environment]::GetFolderPath("MyDocuments")
}
$jsonPath = $MicrosoftUserDir + "\Local State"
...
if ($json.browser -eq $null) {
    ...
    $browser.enabled_labs_experiments = @("enable-lazy-image-loading@3")
    ...
    if ($json.browser.enabled_labs_experiments -eq $null) {
        ...
        $json.browser.enabled_labs_experiments = @("enable-lazy-image-loading@3")
    } else {
        ...
        if (($json.browser.enabled_labs_experiments.GetType()).BaseType.ToString() -eq "System.Array") {
            if ($json.browser.enabled_labs_experiments.Contains("enable-lazy-image-loading@3")) {
